...
class Trainer(object):
    def __init__(self, args):
        ...
        self.args.savepath = args.savepath
        ...
        self.best_monitor = 0.4  # S* score right now
        self.start_epoch = 0
        ...
        #     self.train_loader = data.DataLoader(
        #         DUBAICCDataset(args.network,args.data_folder, args.list_path, 'train', args.token_folder, self.word_vocab, args.max_length, args.allow_unk),
        #         batch_size=args.train_batchsize, shuffle=True, num_workers=args.workers, pin_memory=True)
        #     self.val_loader = data.DataLoader(
        #         DUBAICCDataset(args.network, args.data_folder, args.list_path, 'val', args.token_folder, self.word_vocab, args.max_length, args.allow_unk),
        #         batch_size=args.val_batchsize, shuffle=False, num_workers=args.workers, pin_memory=True)

        # elif args.data_name == 'BD_CC':
        #     self.train_loader = data.DataLoader(
        #         BDCCDataset(args.network,args.data_folder, args.list_path, 'train', args.token_folder, self.word_vocab, args.max_length, args.allow_unk),
        #         batch_size=args.train_batchsize, shuffle=True, num_workers=args.workers, pin_memory=True)
        #     self.val_loader = data.DataLoader(
        #         BDCCDataset(args.network, args.data_folder, args.list_path, 'val', args.token_folder, self.word_vocab, args.max_length, args.allow_unk),
        #         batch_size=args.val_batchsize, shuffle=False, num_workers=args.workers, pin_memory=True)
    ...
    def build_model(self):
        args = self.args
        # Initialize / load checkpoint
        self.encoder = Encoder(args.network, args.data_name, args.embed_dim, args.encoder_dim, args.prompt_type)
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            ...
            monitor = (Bleu_4 + Meteor + Rouge + Cider) / 4
        # Check if there was an improvement
        if monitor > self.best_monitor:
            self.best_monitor = max(monitor, self.best_monitor)
            # save_checkpoint
            print('Save Model')
            ...
            metric = f'S*_{round(100000 * self.best_monitor)}'
            # model_name = f'{self.args.data_name}_bts_{self.args.train_batchsize}_{self.args.network}_epo_{epoch}_{metric}.pth'
            model_name = f'{self.args.data_name}_bts_{self.args.train_batchsize}_{self.args.network}_epo_{epoch}.pth'
            if epoch > 4:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs', type=int, default=40, help='number of epochs to train for (if early stopping is not triggered).')
    parser.add_argument('--workers', type=int, default=4, help='for data-loading; right now, only 0 works with h5pys in windows.')
    parser.add_argument('--encoder_lr', type=float, default=1e-5, help='learning rate for encoder if fine-tuning.')
    ...
    parser.add_argument('--savepath', default="result/1_times/")
    parser.add_argument('--prompt_type', default="sentences")
    # backbone parameters
